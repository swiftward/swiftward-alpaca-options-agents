...
func (w *Watch) consider(ctx context.Context, s Structure, walking map[string]bool) {
	...
	if cost < 0 {
		// Закрытие того, что открыто за КРЕДИТ, не может приносить деньги: такая
		// конструкция стоит от нуля до своей ширины, и выкупить её дешевле нуля
		// нельзя. Отрицательная цена означает, что книга перевёрнута - дальняя
		// нога котируется дороже ближней, чего не бывает, - и одна из котировок
		// протухла.
		//
		// Замерено 28 августа на QQQ 725/726: 726-й колл стоял на покупке 0,03
		// при 725-м на продаже 0,02. Сторож считал выкуп в минус цент, отправлял
		// заявку по невозможной цене и получал отмену по терпению. Одиннадцать
		// раз за два часа, и каждая отмена - строка в записи, которую читает
		// судья.
		//
		// Скринер отказывает на том же основании и теми же словами: структура,
		// которая платит больше своей ширины, - это не находка, а сломанная
		// котировка.

...
func TestThePollerHandsOverATurnAndExitsZero(t *testing.T) {
	...
	turnID, err := m.Turn(context.Background(), "the defence window is open", "sonnet")
	require.NoError(t, err)
	...
	assert.Equal(t, "the defence window is open", got.Text)
...
func TestTheReplyScriptSpeaksAndEndsTheTurn(t *testing.T) {
	...
	_, code := run(t, 20*time.Second, reply, base, "say", turnID, "closed the spread, credit 0.28")
	...
	assert.Equal(t, "closed the spread, credit 0.28", said.Text)

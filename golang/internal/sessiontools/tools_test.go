...
func TestTheSessionManagesItsOwnWakeUps(t *testing.T) {
	...
	res = call("wake_me_on_price", map[string]any{
		"cause": "the price came up to the sold strike", "symbol": "SPY", "direction": "below", "level": 760.0,
	})
	require.False(t, res.IsError, res.Content)
	...
	require.NoError(t, err)
	assert.Contains(t, string(listed), "the sold strike")
...
func TestAWakeUpInThePastIsRefused(t *testing.T) {
	...
	res, err := session.CallTool(context.Background(), &mcp.CallToolParams{
		Name:      "wake_me_at",
		Arguments: map[string]any{"cause": "already past", "at": at.Add(-time.Hour).Format(time.RFC3339)},
	})
	require.NoError(t, err)
...
func TestTheSessionCanReadItsOwnSchedule(t *testing.T) {
	server := httptest.NewServer(Tools{
		...
		Schedule: &scheduleDouble{sessions: []declaration.Scheduled{
			{Name: "entry", Cause: "the entry window", When: "at 14:20, on mon, tue, wed, thu, fri (America/New_York)"},
		}},
	}.Handler())
	t.Cleanup(server.Close)
...
func TestTheSameStructureIsNotRecordedTwiceInOneTurn(t *testing.T) {
	...
	intent := map[string]any{
		"thesis":           "the premium is rich and there is no movement",
...
func TestADifferentStructureInTheSameTurnIsRecorded(t *testing.T) {
	...
	for _, structure := range []string{"SPY 758/757", "QQQ 701/700"} {
		res, err := session.CallTool(context.Background(), &mcp.CallToolParams{
			Name: "record_intent",
			Arguments: map[string]any{
				"thesis": "the premium is rich", "structure": structure, "max_loss": "$90",
				"underlying_price": "612.40",
			},
		})
		require.NoError(t, err)
...
func TestListWakeupsAnswersWithAnObject(t *testing.T) {
	...
	shape := func(what string) {
		...
		require.NoError(t, json.Unmarshal(raw, &object),
			"%s: the answer must be an object, and %s came back", what, raw)
		assert.Contains(t, object, "wakeups", "%s: the list lives under its own name", what)
	}

	shape("nothing is set")

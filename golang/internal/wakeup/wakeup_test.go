...
func TestAWakeUpSurvivesTheProcess(t *testing.T) {
	...
	_, err := store.AddAt("look at how the position opened", now.Add(time.Hour), now)
	require.NoError(t, err)
	_, err = store.AddPrice("the price came up to the sold strike", "spy", Below, 760, now)
	require.NoError(t, err)
	...
	assert.Equal(t, Cause("look at how the position opened"), list[0].Cause)
...
func TestRefusesWhatCannotFire(t *testing.T) {
	...
	_, err := store.AddAt("already past", now.Add(-time.Minute), now)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "not in the future")

	_, err = store.AddPrice("no symbol", "", Below, 760, now)
	require.Error(t, err)

	_, err = store.AddPrice("an unclear side", "SPY", Direction("sideways"), 760, now)
	require.Error(t, err)

	_, err = store.AddPrice("a price that is not a price", "SPY", Below, 0, now)
	require.Error(t, err)
}
...
func TestDueFiresOnceAndForgets(t *testing.T) {
	...
	soon, err := store.AddAt("check the position", now.Add(30*time.Minute), now)
	require.NoError(t, err)
	_, err = store.AddAt("the end of the day", now.Add(4*time.Hour), now)
	require.NoError(t, err)
	...
	assert.Contains(t, due[0].Prompt(), "check the position")
...
func TestPriceWakeUpsFireOnTheirSide(t *testing.T) {
	...
	_, err := store.AddPrice("the price went below the strike", "SPY", Below, 760, now)
	require.NoError(t, err)
	_, err = store.AddPrice("the price came back above", "SPY", Above, 770, now)
	require.NoError(t, err)
...
func TestCancelSaysWhenThereIsNothingToCancel(t *testing.T) {
	...
	created, err := store.AddAt("look later", now.Add(time.Hour), now)
	require.NoError(t, err)
...
func TestIdentifiersDoNotRepeatAfterARestart(t *testing.T) {
	...
	first, err := store.AddAt("one", now.Add(time.Hour), now)
	require.NoError(t, err)

	reopened, err := Open(path)
	require.NoError(t, err)
	second, err := reopened.AddAt("two", now.Add(2*time.Hour), now)
	require.NoError(t, err)
...
func TestAPriceWakeupCarriesNoTime(t *testing.T) {
	...
	priced, err := store.AddPrice("the price came up to the sold strike", "SPY", Below, 600, now)
	require.NoError(t, err)
	timed, err := store.AddAt("look at how the position opened", now.Add(time.Hour), now)
	require.NoError(t, err)

	byPrice, err := json.Marshal(priced)
	require.NoError(t, err)
	assert.NotContains(t, string(byPrice), "0001-01-01")
	assert.NotContains(t, string(byPrice), `"at"`, "a price wake-up carries no time at all")
	...
	assert.Contains(t, string(byTime), `"at":"2026-09-04T14:30:00Z"`,
		"while a timed one must keep it")

	// And what was written to disk reads back the same.

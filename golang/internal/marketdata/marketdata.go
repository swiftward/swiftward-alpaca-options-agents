...
func (b *Broker) ReplaceOrder(ctx context.Context, id string, limit float64, name string) (string, error) {
	...
	// The broker answers a replacement with a NEW order id and keeps the old one
	// only as history. Dropping that id breaks the chain: the steps stay under the
	// order that was replaced, the fill arrives under the one that replaced it, and
	// nothing joins them. Measured 28 August - of 33 fills, 19 could not be traced
	// back to the call that started them.
	var answer struct {
		Data struct {
			ID string `json:"id"`
		} `json:"data"`
	...
	if err := b.call(ctx, "replace_order_by_id", arguments, &answer); err != nil {
		return "", err
	}
...
func (b *Broker) CloseStructure(ctx context.Context, legs []Leg, sets int, limit float64, name string) (string, error) {
	if len(legs) == 0 {
	...
	// The id comes back so the caller can write the order down at the moment it
	// is sent. Without it a closing order exists only at the broker until the
	// ladder happens to notice it, and one cancelled before that pass is in the
	// record nowhere at all - measured 29 August, eight such orders in a day.
	if err := b.call(ctx, "place_option_order", map[string]any{

...
// ServerShell is the server name a shell command is filed under. The agent's
// own protocol gives it no server and no tool name, and everything that decides
// what to do with a shell call - the chat line, the record - asks for this one
// name rather than each spelling it as a literal.
const ServerShell = "shell"
...
func (c Call) Named() string {
	if c.Server == "" || c.Server == ServerShell {
		return c.Tool
	}

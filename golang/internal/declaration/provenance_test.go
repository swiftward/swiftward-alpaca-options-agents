...
//
//	MEASURED       - a measurement stands behind it. Say which script and when.
//	FROM THE RULES - it follows from something outside us: the platform's deadline,
//	                 the broker's own limit. Not measured because there is nothing
//	                 to measure; say which rule.
//	PROVISIONAL    - nobody measured it yet. Say so out loud; it is in the queue.
//
		...
		t.Run(filepath.Base(path), func(t *testing.T) {
			...
			for lines.Scan() {
				...
				if strings.HasPrefix(trimmed, "#") {
					if strings.Contains(line, "MEASURED") ||
						strings.Contains(line, "FROM THE RULES") ||
						strings.Contains(line, "PROVISIONAL") {
						marked = true
					}
			...
			require.Empty(t, bare,
				"these numbers are traded on and say nothing about where they came from: %s.\n"+
					"Put MEASURED (with the script and the date), FROM THE RULES (with the rule), "+
					"or PROVISIONAL in the comment above each.",
				strings.Join(bare, ", "))
		})
	}
}

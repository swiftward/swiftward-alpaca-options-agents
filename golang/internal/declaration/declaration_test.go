...
sessions:
  - name: entry
    cause: "the entry window in the second half of the session"
    task: "Judge the entry conditions and open a position if they line up."
    at: "14:20"
    within: 45m
    days: [mon, tue, wed, thu, fri]
  - name: defend
    cause: "checking the defence rules"
    task: "Check the open positions against the defence rules."
    every: 30m
    between: ["09:40", "15:55"]
`
...
func TestLoadReadsSessions(t *testing.T) {
	...
	assert.Contains(t, d.Sessions[0].Prompt(), "open a position")
...
parameters:
  ...
  min_edge_points: "strictly above 0"
sessions:
  - name: entry
    cause: "the entry window"
    task: "Run premium-harvest."
    at: "14:20"
    within: 45m
`))
	require.NoError(t, err)
	...
	assert.Contains(t, numbers, "min_edge_points = strictly above 0")

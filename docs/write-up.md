...
`at` and `within` mean *fire at 15:35, and still count as due for 20 minutes* - so a restart at 15:40 does not lose the window, and a restart at 18:00 does not close a book nobody asked to have closed. `every` with `between` fires repeatedly inside one: the entry windows run every 10 minutes from 09:45 to 15:15, the defence every 30 from 09:40 to 15:55. `model` names a cheaper model for a session that only reads the news.

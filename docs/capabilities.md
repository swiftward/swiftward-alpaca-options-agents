...
|---|---|---|---|
| A session with a cause | Each session carries a task in words and the reason it was woken; the model decides what to do with it | `agent/alpaca-agent-tikhon.yaml`, `sessions:` | `golang/internal/declaration/declaration_test.go` |
...
| A technique added without a release | A playbook is a `SKILL.md` whose front matter names the numbers it needs, and a declaration grants it by name. What the agent can do is extended by writing a file and naming it in a declaration - no build, no deploy | `agent/skills/`, `golang/internal/skills` | `skills_test.go` |
| A skill that cannot arrive half-configured | An agent whose declaration does not supply a number a granted skill requires does not start, and says which number is missing | `golang/internal/skills`, `Layer.Lay` | `shipped_test.go` |
...
| The numbers live in the declaration | The agent's own trading numbers - the delta ceiling, the edge threshold, the day's fuse, the layer shares - are `parameters:` in its file, and they stand at the head of every turn. The three ceilings that bound a LOSS are deliberately not there: the declaration says so where each number would otherwise stand | `agent/alpaca-agent-tikhon.yaml`, `parameters:` | `golang/internal/api/limits_test.go` |
...
| The agent reads its own record | A session calls `read_state` and gets what earlier sessions did: their turns, the intents they recorded before ordering, and the refusals they were given. So a rule that stopped a trade this morning is in front of the session that trades this afternoon | `golang/internal/sessiontools`, `read_state` | `golang/internal/sessiontools/tools_test.go` |
| Provenance of every number | Every parameter in a declaration is marked `MEASURED`, `FROM THE RULES` or `PROVISIONAL`, and an unmarked one fails the test, so a number that cost a day of computing is told apart from one that cost a minute of taste | `agent/alpaca-agent-tikhon.yaml`, `parameters:` | `golang/internal/declaration/provenance_test.go` |
...
| Closing windows | `flatten` runs daily and carries `cannot_wait`, so it is never queued behind an entry; `deadline-flatten` runs before the result is read and settles the book rather than emptying it | `agent/alpaca-agent-tikhon.yaml`, `sessions:` | `cannotwait_test.go` |
